...
const formSchema = z
  .object({
    ...
    avatar: z.instanceof(File).optional().nullable(), // Add the avatar field to the schema
  })
...
const page = () => {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      email: "",
      phone: undefined,
      avatar: null, // Initialize avatar as undefined
    },
  });
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    try {
      ...
      if (values?.phone) {
        formData.append("phone", values?.phone?.toString()); // Convert to string
      }
      ...
      let response: any = await api.create("/api/users", formData, {
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <Input
                        ...
                        accept="image/*"
                        multiple={false}
                        onChange={(e) => field.onChange(e.target.files?.[0])} // Handle the file change event
                      />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
